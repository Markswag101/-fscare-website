...
const stats = [
  ...
  { icon: <ShieldCheck size={18}/>, value: '100%', label: 'Certified Products' },
        ...
        <div className="hero-content animate-fadeUp">
          ...
          <div className="hero-ctas-2">
            <a href="#contact" className="hero-link">
              <CalendarCheck size={15} /> Book a Consultation
            </a>
            <a href="#contact" className="hero-link">
              <MapPin size={15} /> Find Our Location
            </a>
          </div>
        </div>
            ...
            <div className="card-header">
              <div className="avatar-group">
                {['#3b82f6','#10b981','#f59e0b'].map((c,i) => (
                  <div key={i} className="avatar" style={{background: c, zIndex: 3-i}} />
                ))}
              </div>
              <span className="card-tag">Pharmacists Online</span>
            </div>
            <p className="card-body-text">
              "Speak with a certified pharmacist right now — no waiting, no appointment needed."
            </p>
            <a href="https://wa.me/2347054172448" target="_blank" rel="noreferrer" className="btn btn-primary" style={{fontSize:'13px', padding:'9px 18px'}}>
